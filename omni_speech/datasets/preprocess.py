...
def preprocess_llama_3_v1(
    ...
    unmask_tokens = []
    unmask_tokens_idx = [tokenizer.convert_tokens_to_ids(tok) for tok in unmask_tokens]
    log_file = open("llama_3_v1_ids_log.txt", "a", encoding="utf-8")
    ...
    for i, source in enumerate(sources):
        ...
        input_id += encode_id
        log_file.write(f"encode_id: {str(encode_id)}\n")
        log_file.write(f"input_id: {str(input_id)}\n\n")
...
def preprocess_midm(
    ...
    for i, source in enumerate(sources):
        ...
        input_id += tokenizer.apply_chat_template(all_convs, tokenize=True)
        target += [IGNORE_INDEX] * len(input_id)
        if input_id[-5:] == [131302, 51728, 62350, 131303, 51613]:
            input_id = input_id[:-5]
            target = target[:-5]
        ...
        encode_id = tokenizer.apply_chat_template(conv, tokenize=True)[1:]
        # pdb.set_trace()
        if encode_id[:4] == [131302, 60134, 131303, 51613]:
            if eot_id in encode_id:
                eot_index = encode_id.index(eot_id)
                encode_id = encode_id[eot_index + 1:]
        if encode_id[-5:] == [131302, 51728, 62350, 131303, 51613]:
            encode_id = encode_id[:-5]
        input_id += encode_id
        target += [IGNORE_INDEX] * 5
        target += encode_id[5:]
...
def preprocess(
    ...
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.MIDM:
        return preprocess_midm(sources, tokenizer, has_speech=has_speech)
